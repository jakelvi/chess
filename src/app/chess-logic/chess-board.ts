import { columns } from '../modules/models';
import { FENConverter } from './FENConvreter';
import {
  CheckState,
  Color,
  Coords,
...
export class ChessBoard {
  ...
  private canCaptureEnPassant(
    ...
    const { piece, prevX, prevY, currentX, currentY } = this._lastMove;

    if (
      !(piece instanceof Pawn) ||
      pawn.color !== this._playerColor ||
      Math.abs(currentX - prevX) !== 2 ||
      pawnX !== currentX ||
      Math.abs(pawnY - currentY) !== 1
    )
      return false;

    const pawnNewPositionX: number =
      pawnX + (pawn.color === Color.White ? 1 : -1);
    const pawnNewPositionY: number = currentY;
    ...
    this.chessBoard[currentX][currentY] = piece;
  ...
  private handlingSpecialMoves(
    ...
    if (piece instanceof King && Math.abs(newY - prevY) === 2) {
      ...
      Math.abs(this._lastMove.currentX - this._lastMove.prevX) === 2 &&
      prevX === this._lastMove.currentX &&
      newY === this._lastMove.currentY
    ) {
      this.chessBoard[this._lastMove.currentX][this._lastMove.currentY] = null;
      moveType.add(MoveType.Capture);
    }
  }

  private promotedPiece(
    promtoedPieceType: FENChar
  ): Knight | Bishop | Rook | Queen {
    if (
      promtoedPieceType === FENChar.WhiteKnight ||
      promtoedPieceType === FENChar.BlackKnight
    )
      return new Knight(this._playerColor);

    if (
      promtoedPieceType === FENChar.WhiteBishop ||
      promtoedPieceType === FENChar.BlackBishop
    )
      return new Bishop(this._playerColor);

    if (
      promtoedPieceType === FENChar.WhiteRook ||
      promtoedPieceType === FENChar.BlackRook
    )
  ...
  private storeMove(promotedPiece: FENChar | null): void {
    const { piece, currentX, currentY, prevX, prevY, moveType } =
      this._lastMove!;
    ...
    if (moveType.has(MoveType.Castling))
      move = currentY - prevY === 2 ? 'O-O' : 'O-O-O';
    else {
      ...
      move += columns[currentY] + String(currentX + 1);
  ...
  private startingPieceCoordsNotation(): string {
    ...
    for (let x = 0; x < this.chessBoardSize; x++) {
      for (let y = 0; y < this.chessBoardSize; y++) {
        const piece: Piece | null = this.chessBoard[x][y];
        if (!piece || (currentX === x && currentY === y)) continue;
        ...
        if (piece.FENChar === currPiece.FENChar) {
          ...
          const pieceHasSameTargetSquare: boolean = safeSquares.some(
            (coords) => coords.x === currentX && coords.y === currentY
          );
